...
void reset_buffer(double_buffer* buff) {
    buff->head = 0;
    buff->sum = 0;
}

double_buffer* buff_init(size_t capacity) {
    double_buffer* buff = calloc(1, sizeof(double_buffer));
    buff->buffer = calloc(capacity, sizeof(double));
...
void buff_push(double_buffer* buff, double value) {
    if (buff->full) {
        buff->sum -= buff->buffer[buff->head];
    }
...
size_t buff_size(double_buffer* buff) {
    size_t size = 0;
    if (buff->full) {
        size = buff->capacity;
    } else {
...
float buff_avg(double_buffer* buff) {
    return buff->sum / float(buff_size(buff));
}

double_buffer* iBuff = buff_init(500);
double_buffer* vBuff = buff_init(1480);
...
void EnergyMonitor::loop() {
    ...
    buff_push(iBuff, filteredI * filteredI);
}

bool EnergyMonitor::ready() {
    return iBuff->full;
}
...
double EnergyMonitor::calcIrms()
{
  ...
  double I_RATIO = ICAL *((SupplyVoltage/1000.0) / (ADC_COUNTS));
  Irms = I_RATIO * sqrt(buff_avg(iBuff));
